...
let chkMostrarNubesBajas = localStorage.getItem("METEO_CHECKBOX_MOSTRAR_NUBES_BAJAS") !== "false";
let chkMostrarXC = localStorage.getItem("METEO_CHECKBOX_MOSTRAR_XC") === "true"; // false por defecto
...
function actualizarTextoVisual() {
    ...
    if(display) {
        // --- 1. CONFIGURACIÓN DE ESTILOS (ETIQUETAS) ---
        const estiloBadge = `
            ...
            text-align: center;
        `;
        const crearEtiqueta = (txt) => `<span style="${estiloBadge}">${txt}</span>`;

        // --- 2. PREPARACIÓN ---
        const coordsTexto = `${centroLat.toFixed(4)}, ${centroLon.toFixed(4)}`;
        display.innerHTML = `📍 ${coordsTexto}<br><small>🔄 Consultando idiomas...</small>`; 

        const latSafe = centroLat.toFixed(6);
        const lonSafe = centroLon.toFixed(6);

        // --- 3. PETICIONES PARALELAS ---
        
        // A. LOCAL (Photon)
        const pLocal = fetch(`https://photon.komoot.io/reverse?lon=${lonSafe}&lat=${latSafe}`)
            .then(res => res.ok ? res.json() : null);

        // B. ESPAÑOL (BigDataCloud)
        const pEs = fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${centroLat}&longitude=${centroLon}&localityLanguage=es`)
            .then(res => res.ok ? res.json() : null);

        // C. INGLÉS (BigDataCloud)
        const pEn = fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${centroLat}&longitude=${centroLon}&localityLanguage=en`)
            .then(res => res.ok ? res.json() : null);

        // --- 4. PROCESAMIENTO ---
		Promise.all([pLocal, pEs, pEn])
			.then(resultados => {
				const [dataLocal, dataEs, dataEn] = resultados;

				// A. Procesar LOCAL (Photon)
				let txtLocal = "";
				let provinciaLocal = "";
				if (dataLocal && dataLocal.features && dataLocal.features.length > 0) {
					const props = dataLocal.features[0].properties;
					// 🎯 MEJORA: Añadimos town/village para que no falle fuera de ciudades
					const nombre = props.city || props.town || props.village || props.locality || "";
					const prov = props.state || props.country || ""; 
					
					txtLocal = nombre;
					if (prov && nombre && !nombre.includes(prov)) {
						provinciaLocal = ` | ${prov}`;
					}
				}
				...
				let nombreBaseEs = "";
				if (dataEs) {
					nombreBaseEs = dataEs.locality || dataEs.city || dataEs.town || "";
					const prov = dataEs.principalSubdivision || "";
					if (nombreBaseEs) {
						txtEs = nombreBaseEs + (prov && !nombreBaseEs.includes(prov) ? ` | ${prov}` : "");
					}
				}
				...
				let paisEn = "";
				if (dataEn) {
					nombreBaseEn = dataEn.locality || dataEn.city || dataEn.town || "";
					paisEn = dataEn.countryName || "";
					if (nombreBaseEn) {
						txtEn = nombreBaseEn + (paisEn ? ` | ${paisEn}` : "");
					}
				}

				// --- 5. CONSTRUCCIÓN DEL HTML ---
				let html = `📍 ${coordsTexto}`;
				let ultimaCiudadMostrada = ""; 
				const lineaHr = `<hr style="margin: 5px 0; border: 0; border-top: 1px solid rgba(0,0,0,0.2);">`;

				// Fila 1: LOCAL
				if (txtLocal) {
					html += lineaHr;
					html += `Toponimia automática de la ubicación:<br>(fuentes externas)<br><br>`;
					html += `<div style="display:flex; align-items:center;">${crearEtiqueta("loc")} ${txtLocal}${provinciaLocal}</div>`;
					ultimaCiudadMostrada = txtLocal; 
				}

				// Fila 2: ESPAÑOL
				if (txtEs && nombreBaseEs !== ultimaCiudadMostrada) {
					if (!txtLocal) html += `${lineaHr}Toponimia automática:<br><br>(diversas fuentes)`;
					html += `<div style="display:flex; align-items:center; margin-top:2px;">${crearEtiqueta("es")} ${txtEs}</div>`;
					ultimaCiudadMostrada = nombreBaseEs;
				}

				// Fila 3: INGLÉS (Modificado para que el País fuerce la aparición)
				// Mostramos si la ciudad es distinta O si el texto contiene un país que no hemos mencionado
				if (txtEn) {
					const yaMencionado = html.includes(nombreBaseEn) && html.includes(paisEn);
					if (!yaMencionado) {
						html += `<div style="display:flex; align-items:center; margin-top:2px;">${crearEtiqueta("en")} ${txtEn}</div>`;
					}
				}
			...
			.catch(err => {
				console.warn("Error obteniendo idiomas:", err);
				display.innerText = `📍 ${coordsTexto}`;
			});
    }
}
    ...
    const driverObj = window.driver.js.driver({
        ...
        steps: [
            ...
            { element: '.columna-meteo.borde-grueso-abajo.borde-grueso-arriba.borde-grueso-izquierda', 
                popover: { title: '<div style="display: flex; align-items: center; gap: 8px;"><span style="font-size:25px; display: block;">🌦️</span><span>Columna de meteorología</span></div>', description: 'Muestra los títulos de los datos meteorológicos o análisis ofrecidos: velocidades de viento, racha, dirección y filas opcionales como Cizalladura/Fiabilidad, Rafagosidad y otras.' } },

            { element: '.columna-meteo.columna-simbolo-fija.borde-grueso-izquierda.celda-altura-4px', 
                popover: { title: '🟩🟧🟥 Fila de Cizalladura / Fiabilidad', description: 'Muestra mediante un semáforo de colores el nivel de Cizalladura de Bajo Nivel (LLWS - Low-Level Wind Shear) y también el grado de Fiabilidad del pronóstico de viento medio a 10 m de altura.' , side: 'bottom', align: 'end' } },
            ...
            { element: '.btn-info.btn-abajo-izquierda', 
                popover: { title: '<div style="display: flex; align-items: center; gap: 8px;"><img src="info.svg" width="20" height="20" style="display: block;"><span>Información del despegue</span></div>', description: 'Seleccionando esta <img src="info.svg" width="20" height="20" style="vertical-align: middle; margin-bottom: 2px;"> se muestra información más completa del despegue y un botón para acceder a su mapa.<br><br>👉🏽  El mapa incluye información adicional y varias utilidades que merece la pena explorar.' } },
            ...
            { element: '#btn-div-filtro-distancia-toggle',
                popover: { title: '<div style="display: flex; align-items: center; gap: 8px;"><img src="icono_filtro_60.webp" width="20" height="20" style="display: block;"><span>Filtro de distancia</span></div>', 
            ...
            { element: '#btn-div-configuracion-toggle',
                popover: { title: '⚙️ Configuración', description: 'Te lleva al panel de configuración, donde podrás personalizar parámetros, activar opciones interesantes y ver información de los datos meteorológicos.<br><br>👉🏽 Para cada opción o dato, tienes un botón de información <img src="info.svg" width="20" height="20" style="vertical-align: middle; margin-bottom: 2px;">.' } }
        ],
...
function sugerirGuiaFavoritos(forzar = false) {
    ...
    // ---------------------------------------------------------
    const htmlAyuda = !forzar 
        ? `<br><br>Siempre podrás verla con el botón <img src="icono_ayuda_60.webp" width="20" height="20" style="vertical-align:middle;" alt="Guía">`
        : ''; 
    ...
    const driverObj = window.driver.js.driver({
        ...
        steps: [
            ...
            { element: '#btn-guia-edicion-favoritos',
                popover: { title: '<div style="display: flex; align-items: center; gap: 8px;"><img src="icono_ayuda_60.webp" width="20" height="20" style="display: block;"><span>Guía rápida</span></div>', description: 'Muestra esta guía.' } },

            { element: '#btn-finalizar-edicion-favoritos',
                popover: { title: '🏁 Finalizar edición de favoritos', description: 'Sale a la pantalla normal de la aplicación, con tus favoritos y su pronóstico.<br><br>👉🏽 También puedes volver a esta pantalla cuando quieras ver información <img src="info.svg" style="vertical-align: middle; margin: 0 4px;" width="20" height="20"> de un despegue poco habitual que no te interese tener como favorito permanente.' } }
        ],
...
async function guardarFavoritos() {
    ...
    let nombreArchivo = `Favoritos Fly Decision.txt`;
    ...
    if (isApp) {
        ...
        try {
            ...
            // PASO 1: Guardar copia permanente en DOCUMENTOS
            ...
            const confirmResult = await Dialog.confirm({
                title: '✅ Favoritos guardados en "Documentos" con el nombre:',
            ...
            if (confirmResult.value) {
                await Share.share({
                    ...
                    files: [resultCache.uri], // Telegram leerá esto sin problemas 
...
function gestionarClickMasivoFavoritos() {
    ...
    if (chkMostrarNubesBajas) filasPorDespegue++;
...
function finalizarEdicionFavoritos() {
	...
	if (!localStorage.getItem("METEO_FAVORITOS_LISTA") || favoritos.length === 0) { // Ha pulsado Finalizar pero no existe la key "METEO_FAVORITOS_LISTA" en localstorage o los favoritos están vacíos 
        ...
        mensajeModalAceptar('', 
            '<p>Es necesario marcar al menos un despegue favorito ♥️.</p><p>Si quieres, puedes consultar la guía rápida de esta pantalla con el botón <img src="icono_ayuda_60.webp" width="20" height="20" style="vertical-align:middle;" alt="Guía"></p>'
        );
...
function alternarMostrarNubesBajas() {
    chkMostrarNubesBajas = document.getElementById("chkMostrarNubesBajas").checked;
    localStorage.setItem("METEO_CHECKBOX_MOSTRAR_NUBES_BAJAS", chkMostrarNubesBajas);
    construir_tabla();
}
...
async function construir_tabla(forzarRecarga = false) {
	...
	await new Promise(r => setTimeout(r, 50)); // Pausa técnica. Esto deja al navegador "respirar" unos milisegundos para procesar la clase CSS antes de bloquearse con los cálculos de la tabla.
	...
	try {
        ...
        if (!localStorage.getItem("METEO_PRIMERA_VISITA_HECHA") && 
			...
			const mostrarPaso5 = function() {
				GestorMensajes.mostrar({
					...
					htmlContenido: `
						...
						<p>👉️ Por el momento solo muestra y analiza condiciones de viento. Antes de volar, como sabes, hay que analizar muchos más datos.</p>
			...
			const mostrarPaso4 = function() {
				GestorMensajes.mostrar({
					...
					htmlContenido: `
						...
						<p>✅ <b>Uso básico</b>: selecciona tus despegues favoritos la primera vez y ya tendrás siempre la tabla con sus datos de viento y su análisis automático para cada día.</p>
						...
						<p>✅ <b>Uso avanzado</b>: puedes personalizar en ⚙️ <i>Configuración</i> los límites predeterminados de viento medio (mínimo, ideal y máximo) y rachas máximas. Tus preferencias se usarán para el análisis, coloreado y puntuación de las condiciones. También puedes mostrar una fila coloreada con la <i>rafagosidad</i> y aplicar calibraciones estadísticas automáticas para adaptar el pronóstico a los despegues.</p>
						`,
					botones: [
					{
						texto: '←',
			...
			const mostrarPaso3 = function() {
				GestorMensajes.mostrar({
					tipo: 'modal',
					htmlContenido: `
					<p>💨️ <b>Datos meteorológicos:</b> pronóstico de viento por horas para 4 días (96h). Se actualiza 8 veces al día con los datos oficiales que emite Météo-France de los modelos Arome-HD 1.3km (0-48h) y Arpege 7km (48-96h).</p>
			...
			const mostrarPaso2 = function() {
				GestorMensajes.mostrar({
					tipo: 'modal',
                    htmlContenido: `
                        <p style="font-size: 1.2em; font-weight: bold; text-align:center;">🪂 Fly Decision. Análisis automático del pronóstico de viento para despegues de parapente.</p>
                        <p>🙄 <b>¿Para qué sirve?:</b> para ayudarte a decidir dónde ir a volar. Muestra una tabla con tus despegues favoritos, sus pronósticos de viento y hace un análisis automático de esas <b>condiciones meteorológicas de viento</b>.</p>
                        <p>🧮 <b>¿Qué análisis hace?:</b> compara el pronóstico con los límites de viento medio y racha máxima configurados y con la orientación de cada despegue; también analiza el viento a alturas cercanas al despegue (80, 120 y 180 m), lo que da idea del gradiente y de la fiabilidad del pronóstico.</p>
			...
			const mostrarPaso1 = function() {
                GestorMensajes.mostrar({
                    ...
                    htmlContenido: `
                        ...
                        <p>Pronóstico y análisis automático de viento para despegues de parapente + Mapa de despegues.</p>`,
                    botones: [
                        {
        ...
        const tooltipContentMeteo =
            ...
            "<b>🌦️</b>: Meteo general. Nubosidad (%) y Lluvia<br>" +
            "<b>💦</b>: Precipitación en mm (litros/m²)<br>" +
            "<b>💦?</b>: Probabilidad (%) de que la precipitación supere 0,1 mm (litros/m²).<br>" +
            "<b>☁️↓</b>: Nubosidad baja (%). Porcentaje del cielo cubierto por nubes bajas (del suelo hasta 2000 m de altitud)<br>" +
            ...
            "<img src='icono_racha_48x42.webp' style='width:16px;height:14px;'> : Racha máxima a 10 m del suelo (km/h)<br>" +
            "<img src='icono_direccion_45.webp' style='width:15px;height:15px;'> : Dirección del viento a 10 m del suelo<br>" +

            "<hr style='border:none;border-top:1px solid #000;margin:4px 0;'>" +
            "<img src='icono_cizalla_fiabilidad.png' style='width:30px;height:8px;'> : Cizalladura de Bajo Nivel por velocidad / Fiabilidad del pronóstico" +
            "<hr style='border:none;border-top:1px solid #000;margin:4px 0;'>" +

            "<b>Techo</b>: Altura (km) de la capa límite sobre el suelo (BLH = Boundary Layer Height)<br>" +
		...
		despegues.forEach((d, idx) => {
            ...
            let htmlFilasDesglose = ""; // <-- GUARDARÁ LAS FILAS DE LA TABLA DEL TOOLTIP
            ...
            if (hayDatosMeteo) {
                ...
                try {
                    ...
                    velArray.forEach((velModelo, i) => {
                        ...
                        // --- REGISTRO PARA EL TOOLTIP (Desglose por hora) ---
                        // (Nota: horaStr, diaCorto se declararon más arriba ahora)

                        if (vetoActivado) {
                            let textoVeto = motivoVeto.includes("Lluvia") ? "⛔ Lluvia" : "⛔ Límite superado";
                            htmlFilasDesglose += `
                                <tr style="border-bottom: 1px solid rgba(0,0,0,0.1);">
                                    <td style="padding: 4px; text-align: left; white-space: nowrap;">${horaStr}</td>
                                    <td colspan="4" style="padding: 4px; color: #ff5243; font-weight: bold; text-align: center;" title="${motivoVeto}">${textoVeto}</td>
                                    <td style="padding: 4px; font-weight: bold; color: #ff5243;">0</td>
                                </tr>
                            `;
                        } else {
                            const ratioTotal = ratioCorreccionPorDireccion * ratioCorreccionPorRacha;
                            htmlFilasDesglose += `
                                <tr style="border-bottom: 1px solid rgba(0,0,0,0.1);">
                                    <td style="padding: 4px; text-align: left; white-space: nowrap;">${horaStr}</td>
                                    <td style="padding: 4px;">${ptsDir.toFixed(0)}</td>
                                    <td style="padding: 4px;">${ptsVel.toFixed(0)}</td>
                                    <td style="padding: 4px;">${ptsRacha.toFixed(0)}</td>
                                    <td style="padding: 4px; font-size: 0.85em; color: #666;">x${ratioTotal.toFixed(2)}</td>
                                    <td style="padding: 4px; font-weight: bold;">${ptsHora.toFixed(0)}</td>
                                </tr>
                            `;
                        }
            ...
            let filaPreci, filaProbPreci, filaNubesBajas;
            ...
            if (chkMostrarNubesBajas) filaNubesBajas = document.createElement("tr");
            ...
            const rowsGroup1 =[filaNubesTotal, filaPreci, filaProbPreci, filaNubesBajas, fila180, fila120, fila80, filaVel, filaRafagosidad, filaRacha, filaDir, filaCizalladura].filter(Boolean);
			...
			const botonMapaHTML = `
				<button class="btn-info btn-abajo-izquierda" 
					...
					<img src="info.svg" alt="Más info" style="width: 20px; height: 20px; vertical-align: middle;">
				</button>
			`;
			...
			if (!modoEdicionFavoritos) {
                ...
                addIconCell(filaPreci, '<span style="font-size:16px; font-weight:bold;">💦</span>', 'Precipitación en mm (litros/m²)');
                addIconCell(filaProbPreci, '<span style="font-size:16px; font-weight:bold;">💦?</span>', 'Probabilidad (%) de que la precipitación supere 0,1 mm (litros/m²)');
                addIconCell(filaNubesBajas, '<span style="font-size:16px; font-weight:bold;">☁️↓</span>', 'Nubosidad baja (%): Porcentaje del cielo cubierto por nubes bajas (del suelo hasta 2000 m de altitud)');
				...
				tdIconoRacha.innerHTML = '<img src="icono_racha_48x42.webp" width="16" height="14">';
				...
				tdIconoDireccion.innerHTML = '<img src="icono_direccion_45.webp" width="15" height="15">';
				...
				if (chkMostrarCizalladura) {
				    ...
				    tdIconoCiz.setAttribute("title", "Cizalladura de Bajo Nivel por velocidad / Fiabilidad del pronóstico");	
                ...
                // Iconos Grupo 2: XC
                addIconCell(filaTecho, '<span style="font-size:10px; font-weight:bold;">Techo</span>', 'Techo de vuelo (km): Altura sobre el suelo de la capa límite (BLH = Boundary Layer Height)');
				...
				if (hayDatosMeteo) {
                    ...
                    if (hourlyEcmwf) {
                        ...
                        renderEcmwfData(filaNubesBajas, hourlyEcmwf.cloud_cover_low, 
                        ...
                        renderEcmwfData(filaNubesBajas, emptyArr, () => "", "9px", () => "");
                    }
					...
					velocidades.forEach((velocidadModelo, i) => {
                        ...
                        // --- 📊 GENERACIÓN DEL TOOLTIP GRÁFICO (MONOSPACE) ---

                        // 1. Preparar datos numéricos seguros (null si no hay dato)
                        // Aseguramos que sean números para evitar errores en cálculos matemáticos
                        const n10  = Number(velocidad); 
                        const n80  = (vel80[i] !== undefined) ? Math.round(Number(vel80[i])) : null;
                        const n120 = (vel120[i] !== undefined) ? Math.round(Number(vel120[i])) : null;
                        const n180 = (vel180[i] !== undefined) ? Math.round(Number(vel180[i])) : null;

                        // 2. Cabecera con información textual (Calibración)

                        const textoFechaPrediccion = cacheTextosFecha[i];

                        let htmlCabecera = "";
                        if (chkAplicarCalibracion) {
                            htmlCabecera = `<div style="margin-bottom:8px; padding-bottom:6px; border-bottom:1px solid #ddd; text-align:left;">
                                            <div style="color:#0078d4; font-size:0.95em; margin-bottom:6px; font-weight:bold;">📅 ${textoFechaPrediccion}</div>
                                            <b>${n10} km/h a 10 m</b> (calibrada)<br>
                                            <span>${Math.round(velocidadModelo)} km/h a 10 m (original)</span>
                                            </div>`;
                        } else {
                            htmlCabecera = `<div style="margin-bottom:8px; padding-bottom:6px; border-bottom:1px solid #ddd; text-align:left;">
                                            <div style="color:#0078d4; font-size:0.95em; margin-bottom:6px; font-weight:bold;">📅 ${textoFechaPrediccion}</div>
                                            <b>${n10} km/h a 10 m</b> (original)
                                            </div>`;
                        }

                        // 3. Función auxiliar SOLO para pintar caracteres (el color se decide fuera)
                        const generarBarraAscii = (val) => {
                            if (val === null) return "";
                            // Escala visual: 1 carácter por cada 3 km/h (máx 15 caracteres)
                            const longitud = Math.min(15, Math.ceil(val / 3));
                            return "▬".repeat(longitud);
                        };

                        // 4. Construcción del "Gráfico" ASCII en HTML
                        let htmlGrafico = `<div style="font-family: 'Menlo', 'Consolas', 'Monaco', monospace; font-size: 16px; white-space: pre; text-align:left; line-height: 1.6; color:#333;">`;
                        
                        htmlGrafico += `<span style="color:#999;">Altura | Velocidad</span><br>`;

                        const niveles = [
                            { txt: "180 m", val: n180 }, // Quitamos espacios extra en key para coincidir con objeto LIMITES
                            { txt: "120 m", val: n120 },
                            { txt: " 80 m", val: n80  }, // El espacio se añade visualmente en el padStart luego
                            { txt: " 10 m", val: n10  }
                        ];

                        // CONSTANTES GLOBALES DE LÓGICA (Idénticas a la tabla)
                        const UMBRAL_TURBULENCIA_BAJA = 25; 
                        const valSueloParaCalculo = Math.max(8, n10); // Suelo Virtual

                        niveles.forEach(nivel => {
                            if (nivel.val !== null) {
                                let color = "#19ed86"; // Verde por defecto
                                let icon = "";

                                // --- LÓGICA DE COLOR (ADAPTADA) ---
                                
                                // CASO ESPECIAL: SUELO (10 m)
                                // El suelo se compara directamente con tu VelocidadMax, sin ratios.
                                if (nivel.txt.includes("10 m")) {
                                    if (nivel.val >= VelocidadMax) {
                                        color = "#ff5243"; // Rojo
                                    } else if (nivel.val >= (VelocidadMax - (VelocidadMax - VelocidadIdeal)/1.5)) {
                                        color = "#ffa500";
                                    }
                                } 
                                // CASO ALTURAS (80, 120, 180)
                                else {
                                    const alturaKey = nivel.txt.trim(); // "180 m"
                                    const limites = LIMITES_CIZALLADURA[alturaKey];
                                    const factorSeguridad = limites ? limites.naranja : 1.0;
                                    const limitUser = Math.round(VelocidadMax * factorSeguridad);
                                    
                                    const ratio = nivel.val / valSueloParaCalculo;

                                    // 1. Prioridad: ¿Supera tu límite personal?
                                    if (nivel.val >= limitUser) {
                                        color = "#ff5243";
                                    }
                                    // 2. Prioridad: ¿Es viento flojo (<25)? -> Verde
                                    else if (nivel.val < UMBRAL_TURBULENCIA_BAJA) {
                                        color = "#19ed86"; 
                                    }
                                    // 3. Prioridad: Cizalladura
                                    else if (limites) {
                                        if (ratio >= limites.rojo) color = "#ff5243";
                                        else if (ratio >= limites.naranja) color = "#ffa500";
                                    }
                                }

                                // Generamos la barra visual
                                const barra = generarBarraAscii(nivel.val);
                                
                                // Formateo de texto (Alineación)
                                const etiqueta = nivel.txt.padStart(6, ' '); 
                                const valor    = String(nivel.val).padStart(3, ' ');
                                
                                htmlGrafico += `${etiqueta} | <span style="color:${color}; font-weight:bold; letter-spacing:-1px;">${barra}</span> ${valor} km/h ${icon}<br>`;
                            }
                        });

                        htmlGrafico += `</div>`;

                        // 5. ASIGNACIÓN A TIPPY
                        td.setAttribute("data-tippy-content", htmlCabecera + htmlGrafico);
                        filaVel.appendChild(td);
                        // FIN 📊 GENERACIÓN DEL TOOLTIP GRÁFICO (MONOSPACE) ---
					});
					...
					if (chkMostrarCizalladura) {
					    ...
					    arr10.slice(0, horas.length).forEach((vel10Raw, i) => {
					        ...
					        const tooltipHTML = `
					            <div style="text-align: left; line-height: 1.4;">
					                <div style="color:#0078d4; margin-bottom:6px; font-weight:bold;">📅 ${textoFechaPrediccion}</div><br>
                                    <div style="display: flex; align-items: center; gap: 6px;">
                                            <span>${iconoResultado}</span> 
                                            <b>${textoResultado}</b>
                                    </div>
                                    <br>
                                    <div style="margin-bottom: 15px; padding: 8px; border: 1px solid #ccc; border-radius: 4px; background-color: #fff;">
                                        <div>Motivo: ${motivoCalculo}</div>
                                    </div>
					                
					                <span style="display:block; margin-top:4px; background:#dddddd; padding:6px; border-radius:4px;">
					                • Velocidad máxima en alturas bajas: <b>${vientoMaxAltura}</b> km/h<br>
					                • Diferencia con velocidad 10 m (Δ): <b>${delta > 0 ? '+'+delta : delta}</b> km/h<br>
					                • Proporción (Ratio): <b>${ratio.toFixed(1)}x</b> (mín. 8 km/h)
					                </span>
					            </div>
					        `;
					        td.setAttribute("data-tippy-content", tooltipHTML);
                    ...
                    if (hourlyEcmwf) {
                        ...
                        renderEcmwfData(filaTecho, hourlyEcmwf.boundary_layer_height, 
                            v => v == null ? "" : ((Number(v) / 1000).toFixed(1) === "0.0" ? "" : (Number(v) / 1000).toFixed(1)), "12px",
                            v => v == null ? "" : (Number(v) < 1000 ? "fondo-rojo" : (Number(v) <= 2000 ? "fondo-naranja" : "fondo-verde")),
                            "0px",
                            v => v == null ? "" : "Techo: " + Math.round(Number(v)) + " m sobre el suelo (AGL)"
                        );
                        // CAPE
            ...
            if (horasValidas > 0) {
                const maximosPuntosPosibles = horasValidas * 100;
                const formulaExplicacion = `
                    <div style="text-align: left; line-height: 1.4; cursor: default;">
                        <div style="color:#0078d4; font-size:1.1em; margin-bottom:6px; font-weight:bold;">⭐ Desglose de Puntuación</div>
                        
                        <div style="background: #f9f9f9; padding: 8px; border-radius: 6px; margin-bottom: 10px; border: 1px solid #e0e0e0;">
                            <b>Fórmula base (máximo 100 puntos/hora):</b><br>
                            (Dir [50] + Vel [20] + Rach [30]) × Modificadores
                        </div>

                        <div style="margin-bottom: 10px;">
                            Total acumulado: <b>${puntosAcumulados.toFixed(0)}</b> puntos<br>
                            Puntos posibles (${horasValidas}h): <b>${maximosPuntosPosibles}</b> puntos<br>
                            Nota final: (${puntosAcumulados.toFixed(0)} / ${maximosPuntosPosibles}) × 10 = <b style="color:#000;">${notaFinal.toFixed(1)} puntos</b>
                        </div>

                        <table class="tabla-tooltip-desglose" style="width: 100%; border-collapse: collapse; text-align: center;">
                            <thead>
                                <tr style="background-color: #eef2f5; border-bottom: 2px solid #ccc;">
                                    <th style="padding: 6px 4px; text-align:left;">Hora</th>
                                    <th style="padding: 6px 4px;" title="Puntos Dirección (máx 50)">Dir</th>
                                    <th style="padding: 6px 4px;" title="Puntos Velocidad (máx 20)">Vel</th>
                                    <th style="padding: 6px 4px;" title="Puntos Racha (máx 30)">Rach</th>
                                    <th style="padding: 6px 4px;" title="Factor Modificador">Mod</th>
                                    <th style="padding: 6px 4px;" title="Total Hora">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${htmlFilasDesglose}
                ...
                `;
                tdCondiciones.setAttribute("data-tippy-content", formulaExplicacion);
            } else {
                tdCondiciones.setAttribute("data-tippy-content", "Sin datos suficientes para puntuar");
            }
		...
		// 1. Primero damos la orden de ocultar el cargador para que empiece el "fade out"
		ocultarLoading();

		// 2. Esperamos 100ms a que el cargador sea un poco transparente y lanzamos el scroll suave
		setTimeout(() => {

            // Si hemos llegado hasta aquí, es que NO ha habido crash. Borramos la bandera de seguridad.
            localStorage.removeItem('METEO_FLAG_CRASH_DETECTADO');
            localStorage.removeItem('METEO_CRASH_COUNTER');
			
			const scrollOptions = { top: 0, behavior: 'smooth' }; // 'smooth' es la clave para que sea lento/animado
			...
			// Intento 2: Contenedor Principal
			const principal = document.querySelector('.contenedor-principal-tabla');
			if (principal) {
				principal.scrollTo(scrollOptions);
			}

			// Intento 3: Window (fallback)
			window.scrollTo(scrollOptions);

		}, 100); // Pequeño retraso para asegurar que el usuario ve la animación empezar
            ...
            if (visibles < totalDespeguesDisponibles) {
                ...
                const htmlNumeroFiltrado = `
                    <span class="contador-badge-filtro" title="Filtro activo">
                        <img src="icono_filtro_39.webp" width="13" height="13" alt="Filtro">
                        <b>${visibles}</b>
                    </span>`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (condicionesSlider) {
            ...
            pips: {
                ...
                format: {
                    to: function(val) {
						...
						if (val === 11) {
							return `<img src="icono_parapente.png" style="width:20px;height:20px; vertical-align: middle;">`;
						}
						return ""; 
					}
                }
		...
		condicionesSlider.noUiSlider.on('change', function(values) {
			...
			if (valorNuevo !== ultimaCondicionConfirmada) {
				...
				construir_tabla(); // <--- Los "enormes cálculos" solo ocurren aquí
			}
		});
    ...
    function refrescoPanelInfoActualizaciones() {
        ...
        // LÓGICA MAESTRA VISUAL:
        // Mostramos error si: (Cargamos caché vieja al arrancar) O (El Gestor ha confirmado corte de 1 min)
        const mostrarErrorOffline = esModoOffline || avisoOfflineActivo;

        // --- A. PANEL CENTRAL DE DATOS ---
        ...
        if (dataGenElement) {
            ...
            if (mostrarErrorOffline) {
                ...
                // Cálculo de tiempo para el mensaje
                ...
                dataGenElement.innerHTML = `<span style="color: #ff8400; font-weight: bold;">⚠️ No hay conexión a Internet. Los datos meteorológicos que se muestran tienen una antigüedad de: ${timeAgoGen}.</span>`;
            } else if (lastDataGenerationTimestamp > 0 && !hayErrorData) {
                // Modo Normal
                const timeAgoGen = typeof formatTimeAgo === 'function' ? formatTimeAgo(lastDataGenerationTimestamp, ahoraMs) : '';
                let jsonRefUTC = (jsonModelInitTimestamp > 0 && typeof formatHourUTC === 'function') ? formatHourUTC(new Date(jsonModelInitTimestamp)) : '';
                dataGenElement.innerHTML = `&nbsp;&nbsp;Meteo actualizada hace <b>${timeAgoGen}</b> <span style="color:#777; font-style:italic;">(ref. ${jsonRefUTC}Z)</span>`;
            } else if (!hayErrorData) {
                dataGenElement.textContent = 'Cargando...';
            }
        }

        // --- B. STATUS (PIE DE PÁGINA) ---
        const statusElement = document.getElementById('meteo_status');
        if (statusElement) {
            if (mostrarErrorOffline) {
                statusElement.textContent = "Modo offline. Esperando conexión...";
                statusElement.style.color = '#ff8400';
            } else {
                statusElement.textContent = formatearTextoStatus(currentStatusText);
                // Colores según el texto que venga del servidor
                const upperText = currentStatusText.toUpperCase();
                if (upperText.includes("OPERATIVO") || upperText.includes("OK")) statusElement.style.color = 'green';
                else if (upperText.includes("ERROR") || upperText.includes("FATAL")) statusElement.style.color = 'red';
                else statusElement.style.color = '#ff8400'; 
            }
        }
        ...
        // --- D. PRÓXIMA ACTUALIZACIÓN ---
        const proximaActualizacionContenedor = document.getElementById('proxima-actualizacion-contenedor');
        const proximaActualizacion = document.getElementById('proxima-actualizacion');
        
        if (proximaActualizacion && proximaActualizacionContenedor) {
            if (statusActualizaciónEnCurso || mostrarErrorOffline) {
                proximaActualizacionContenedor.style.display = 'none';
            } else {
                proximaActualizacionContenedor.style.display = 'block';
                // (Aquí va tu lógica de cálculo de tiempo original, resumida para no ocupar espacio)
                let proximaFecha = null;

                // Definimos un margen de tolerancia (ej: 45 minutos).
                // Si la hora prevista está a menos de 45 min de los datos que YA tenemos, 
                // consideramos que esa actualización ya está cumplida.
                ...
                for (let h of HorariosMediosActualizacion) {
                    ...
                    //if (intento > ahora) { proximaFecha = intento; break; }

                    // LÓGICA CORREGIDA:
                    // 1. Tiene que ser futuro (intento > ahora)
                    // 2. Y ADEMÁS: La diferencia entre ese "futuro" y los datos actuales 
                    //    tiene que ser mayor que el margen.
                    let distanciaConDatosActuales = Infinity;
                    if (lastDataGenerationTimestamp > 0) {
                        distanciaConDatosActuales = intento.getTime() - lastDataGenerationTimestamp;
                    }

                    if (intento > ahora && distanciaConDatosActuales > MARGEN_TOLERANCIA_MS) { 
                        proximaFecha = intento; 
                        break; 
                    }
                }
                if (!proximaFecha) {
                    const [hora, min] = HorariosMediosActualizacion[0].split(':').map(Number);
                    proximaFecha = new Date(ahora);
                    proximaFecha.setUTCDate(proximaFecha.getUTCDate() + 1); 
                    proximaFecha.setUTCHours(hora, min, 0, 0);
                }

                const OFFSET_MS = 6 * 60 * 1000; // +6 min

                const diffMs = (proximaFecha - ahora) + OFFSET_MS;
                const diffMins = Math.floor(diffMs / 60000) % 60;
                const diffHoras = Math.floor(Math.floor(diffMs / 60000) / 60);
                let texto = diffHoras > 0 ? `~${diffHoras} h ${diffMins} min` : `~${diffMins} min`;
                proximaActualizacion.innerHTML= `&nbsp;&nbsp;Próxima actualización en <b>${texto}</b>`;
            }
        } 
    }
    ...
    async function PanelInfoActualizaciones_Web() {
        // Silenciador de consola: Si el navegador sabe que no hay red, no intentamos fetch
        if (!navigator.onLine) return; 

        try {
            const response = await fetch("https://flydecision.com/json_timestamp_and_model_run_ref_time.txt?t=" + Date.now(), { cache: "no-store" });
            ...
            console.warn("Error timestamp:", e.message);
            // No cambiamos la UI aquí, dejamos que el Gestor decida si es offline
        }
    }

    async function PanelInfoActualizaciones_Status_auto_actualizaciones() {
        if (!navigator.onLine) return 60000; // Si offline, ritmo lento

        let nuevoIntervalo = 60000;
        try {
            const response = await fetch('https://flydecision.com/meteo-status.txt?t=' + Date.now());
            if (!response.ok) throw new Error('No status file');
            ...
            // Análisis de Status
            if (upperText.includes("ERROR") || upperText.includes("FATAL") || upperText.includes("FAILED")) {
                 nuevoIntervalo = 60000; 
            } else if (upperText.includes("OK") || upperText.includes("OPERATIVO")) {
                if (statusActualizaciónEnCurso) {
                ...
                statusActualizaciónEnCurso = true;
                nuevoIntervalo = 5000; // Modo rápido (actualizando)
            }

            // ✅ ÉXITO: Si llegamos aquí, hay internet REAL. Le confirmamos a tu gestor de conexión que todo va bien. Esto cancela cualquier cuenta atrás de desconexión que pudiera haber pendiente.
            gestionarCambioConexion('online');

        } catch (e) {
            // Si falla el fetch pero el Gestor aún no ha declarado offline, mantenemos el último texto
             console.warn("Fallo fetch status");
             // ❌ FALLO: Capacitor dice que hay WiFi, pero el servidor no responde. Esto es el "Efecto Túnel". Aquí avisamos manualmente.
    ...
    if (document.getElementById("chkMostrarNubesBajas")) document.getElementById("chkMostrarNubesBajas").checked = chkMostrarNubesBajas;
